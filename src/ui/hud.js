...
export class Hud {
  ...
  _buildSettings() {
    ...
    const timeRow = this._slider('Time of day', 'timeOfDay', 0, 1, 0.005, clockLabel, undefined, () => {
    ...
    light.append(
      this._toggle('Panel glint', 'solarGlint', 'Let live output glint on the power zone’s solar array.'),
      this._toggle(
        'Environment light',
        'ibl',
  ...
  setLegend(projects, activeName = null, hidden = [], folded = []) {
    const signature =
      projects.map((p) => `${p.name}:${p.count}:${p.accent}:${p.urgent ? 1 : 0}:${p.seen || ''}`).join('|') +
      `~${activeName}~` +

 ...
 *
 *   1. `opencode serve` HTTP API (OpenCode Desktop runs on the WinPC).
 *      Desktop binds loopback-only (127.0.0.1) on a RANDOM port with an
 *      AUTOGENERATED password — both rotate every Desktop launch — so the
 *      colony reaches it through a read-only SSH tunnel, never over the LAN:
